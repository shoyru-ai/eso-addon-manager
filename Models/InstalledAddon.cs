...
public class InstalledAddon : ObservableObject
{
    ...
    /// <summary>For Shoyru's custom/published addons (not on ESOUI): the manifest download URL. When set,
    /// this addon updates from the published manifest just like an ESOUI addon (Installed tab, Update All,
    /// Pro auto-update) instead of being reachable only through the gated Custom Addons tab.</summary>
    private string _customDownloadUrl = "";
    public string CustomDownloadUrl
    {
        get => _customDownloadUrl;
        set { if (SetProperty(ref _customDownloadUrl, value)) { OnPropertyChanged(nameof(IsCustom)); OnPropertyChanged(nameof(UpdateAvailable)); OnPropertyChanged(nameof(ShowUpdate)); } }
    }
    /// <summary>True for a Shoyru published/custom addon (updates via CustomDownloadUrl, not ESOUI).</summary>
    public bool IsCustom => !string.IsNullOrEmpty(_customDownloadUrl);
    ...
    /// <summary>True when the newest known version (ESOUI catalog for managed addons, or the published
    /// manifest for Shoyru's custom addons) is numerically newer than what's installed.</summary>
    public bool UpdateAvailable =>
        (Managed || IsCustom) && !string.IsNullOrWhiteSpace(LatestVersion) && VersionCompare.IsNewer(LatestVersion, Baseline);
