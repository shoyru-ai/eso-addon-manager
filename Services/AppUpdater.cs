...
public static class AppUpdater
{
    ...
    public static async Task<bool> DownloadAndApplyAsync(string exeUrl)
    {
        ...
        Diag.Log($"AppUpdater: processPath={current}");
        ...
        Diag.Log($"AppUpdater: downloaded {bytes.Length} bytes");
...
echo helper: process exited, swapping >> ""{log}""
copy /Y ""{tempExe}"" ""{current}"" >nul
echo helper: copy errorlevel=%errorlevel% >> ""{log}""
start """" ""{current}""
echo helper: relaunched >> ""{log}""
del ""{tempExe}"" >nul 2>&1
(goto) 2>nul & del ""%~f0""
";
        ...
        Diag.Log("AppUpdater: helper started");
        return true;
    }
}
